...
// ─────────────────────────────────────────
const state = {
  tab: 'exercises',   // 'exercises' | 'workouts' | 'settings'
  screen: null,       // null=list | 'form' | 'editor' | 'detail'
  id: null,
};
...
// Trigger a full re-render of the current screen (used after lang change, import)
...
async function render() {
  ...
  // Nav labels (update for language)
  ...
  if (state.tab === 'exercises') {
    if (!state.screen) {
      await renderExerciseList(content, navigate);
    } else if (state.screen === 'form') {
      await renderExerciseForm(content, navigate, state.id);
    }

  } else if (state.tab === 'workouts') {
    if (!state.screen) {
      await renderWorkoutList(content, navigate);
    } else if (state.screen === 'editor') {
      await renderWorkoutEditor(content, navigate, state.id);
    } else if (state.screen === 'detail') {
      await renderWorkoutDetail(content, navigate, state.id);
    }
...
// ─────────────────────────────────────────
// Generic modal (exercise picker, etc.)
// ─────────────────────────────────────────
